...
kubectl apply -k "$ROOT_DIR/k8s/docker-desktop"
...
if [[ "$scaled_up" != "true" ]]; then
  echo "Worker deployment did not scale above zero before timeout" >&2
  "$ROOT_DIR/scripts/docker-desktop-local/collect-audit.sh" "$upload_id" || true
  exit 1
fi
...
for _ in {1..120}; do
  ...
  if [[ "$status" == "SUCCESS" || "$status" == "SUCCESS_WITH_ERRORS" ]]; then
    ...
    "$ROOT_DIR/scripts/docker-desktop-local/collect-audit.sh" "$upload_id"
