...
public class Tenant1ExtVehicleAction extends AbstractSTMTransitionAction<VehicleExtensionTenant1, MinimalPayload> {
    ...
    @Override
    public void transitionTo(VehicleExtensionTenant1 vehicle, MinimalPayload payload, State startState, String eventId,
                             State endState, STMInternalTransitionInvoker<?> stm, Transition transition) throws Exception {
        applyExtComment(vehicle, payload);
        publishWorkflowEvent(vehicle, payload);
    }

    private void applyExtComment(VehicleExtensionTenant1 tenant1Vehicle, MinimalPayload payload) {
        String comment = payload == null ? null : payload.getComment();

            //tenant1Vehicle.closeComment = comment;
            tenant1Vehicle.tenant1WorkflowNote = comment == null ? null : "tenant1-workflow-" + comment;
    }

    private void publishWorkflowEvent(VehicleExtensionTenant1 vehicle, MinimalPayload payload) {
        if (chenilePub == null) {
            return;
        }
